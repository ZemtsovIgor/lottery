...
const TBCCLottery = artifacts.require("./TBCCLottery.sol");
...
contract(
  ...
  let _priceTicketInTBCC = parseEther("0.5");
  ...
  // Contracts
  let lottery, mockTBCC, randomNumberGenerator;
  ...
  before(async () => {
    // Deploy MockTBCC
    mockTBCC = await MockERC20.new("Mock TBCC", "TBCC", _totalInitSupply);

    // Deploy MockRandomNumberGenerator
    randomNumberGenerator = await MockRandomNumberGenerator.new({ from: alice });

    // Deploy TBCCLottery
    lottery = await TBCCLottery.new(mockTBCC.address, randomNumberGenerator.address, { from: alice });
  ...
  describe("LOTTERY #1 - CUSTOM RANDOMNESS", async () => {
    ...
    it("Users mint and approve TBCC to be used in the lottery", async () => {
      for (let thisUser of [alice, bob, carol, david, erin, injector]) {
        await mockTBCC.mintTokens(parseEther("100000"), { from: thisUser });
        await mockTBCC.approve(lottery.address, parseEther("100000"), {
          from: thisUser,
        });
      }
    });
    ...
    it("Operator starts lottery", async () => {
      ...
      expectEvent(result, "LotteryOpen", {
        ...
        priceTicketInTBCC: _priceTicketInTBCC.toString(),
        firstTicketId: "0",
        injectedAmount: "0",
      });
    ...
    it("Bob buys 100 tickets", async () => {
      ...
      expectEvent.inTransaction(result.receipt.transactionHash, mockTBCC, "Transfer", {
        from: bob,
        to: lottery.address,
    ...
    it("Owner does 10k TBCC injection", async () => {
    ...
    it("David claims the jackpot", async () => {
      // 10,000 + 47.525 + 0.5 + 4.9775 = 10053.0025 TBCC collected
      // 10053.0025 * (1-0.20) * 0.5 = 4021.201 TBCC
    ...
    it("Bob claims 10 winning tickets he bought", async () => {
      // 10,000 + 47.525 + 0.5 + 4.9775 = 10053.0025 TBCC collected
      // 10053.0025 * (1-0.20) * 0.02 = 160.84804 TBCC
      // 10053.0025 * (1-0.20) * 0.03 = 241.27206 TBCC
    ...
    describe("LOTTERY #2 - CUSTOM RANDOMNESS - Exceptions", async () => {
      ...
      it("Operator cannot start lottery if length is too short/long", async () => {
        ...
        await expectRevert(
          lottery.startLottery(endTimeTarget, _priceTicketInTBCC, _discountDivisor, _rewardsBreakdown, _treasuryFee, {
            from: operator,
          }),
      ...
      it("Operator cannot start lottery if discount divisor is too low", async () => {
        ...
        await expectRevert(
          lottery.startLottery(endTime, _priceTicketInTBCC, _discountDivisor, _rewardsBreakdown, _treasuryFee, {
            from: operator,
          }),
      ...
      it("Operator cannot start lottery if ticket price too low or too high", async () => {
        let newPriceTicketInTBCC = parseEther("0.0049999999");

        await expectRevert(
          lottery.startLottery(endTime, newPriceTicketInTBCC, _discountDivisor, _rewardsBreakdown, _treasuryFee, {
            from: operator,
          }),
          "Outside of limits"
        );

        newPriceTicketInTBCC = parseEther("0.0049999999");
      ...
      it("Operator cannot start a second lottery", async () => {
        await expectRevert(
          lottery.startLottery(_lengthLottery, _priceTicketInTBCC, _discountDivisor, _rewardsBreakdown, _treasuryFee, {
            from: operator,
          }),
    ...
    describe("Role exceptions", async () => {
      it("Owner can recover funds only if not TBCC token", async () => {
        // Deploy Random Token
        ...
        await expectRevert(
          lottery.recoverWrongTokens(mockTBCC.address, parseEther("1"), { from: alice }),
          "Cannot be TBCC token"
        );
      });
      ...
      it("Only owner/injector can call owner functions", async () => {
        ...
        await expectRevert(
          lottery.recoverWrongTokens(mockTBCC.address, parseEther("10"), { from: operator }),
          "Ownable: caller is not the owner"
        );
